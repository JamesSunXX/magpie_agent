...
\`\`\`
重要：openQuestions 必须从各领域 partial TRD 中提取所有 P0 阻塞项、待确认问题、未解决的架构决策和风险项。逐领域扫描评审结论中的行动项表格，将每个需要人工确认或决策的条目提取为一条 openQuestion。P0 阻塞项设为 blocker=true、priority="high"；P1 设为 blocker=false、priority="medium"；P2 设为 priority="low"。如果某领域评审结论为 Block，该领域的所有 P0 条目必须出现在 openQuestions 中。禁止返回空数组。
只输出 JSON，不要额外解释。`
