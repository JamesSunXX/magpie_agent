...
async function selectReviewers(): Promise<string[]> {
  ...
  console.log()
  const answer = await question(chalk.white('Enter reviewer numbers separated by comma (e.g., 1,2): '))
  rl.close()
  ...
  const selections = answer
    ...
    .filter(n => Number.isFinite(n) && n >= 1 && n <= AVAILABLE_REVIEWERS.length)
...
async function selectNotificationOptions(): Promise<InitNotificationsOptions | undefined> {
  ...
  const feishuWebhookUrl = (await question(chalk.white('Feishu webhook URL [${FEISHU_WEBHOOK_URL}]: '))).trim()
  const feishuWebhookSecret = (await question(chalk.white('Feishu webhook secret [${FEISHU_WEBHOOK_SECRET}]: '))).trim()
  const imessageAppleScriptTargetsRaw = (await question(chalk.white('iMessage local targets (comma-separated) [handle:+8613800138000]: '))).trim()
  const imessageBluebubblesServerUrl = (await question(chalk.white('BlueBubbles server URL [${BLUEBUBBLES_SERVER_URL}]: '))).trim()
  const imessageBluebubblesPassword = (await question(chalk.white('BlueBubbles password [${BLUEBUBBLES_PASSWORD}]: '))).trim()
  const imessageBluebubblesChatGuid = (await question(chalk.white('BlueBubbles chat guid [${BLUEBUBBLES_CHAT_GUID}]: '))).trim()
...
export const initCommand = new Command('init')
  ...
  .action(async (options) => {
    try {
      ...
      const result = initConfigWithResult(undefined, selectedReviewers, { notifications: notificationOptions })
