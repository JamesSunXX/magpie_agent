  ...
  .action((options: ReviewerListOptions) => {
    try {
      ...
      if (reviewers.length === 0) {
        if (options.model) {
          console.log(chalk.yellow(`No reviewers configured with model "${options.model}"`))
        } else {
          console.log(chalk.yellow('No reviewers configured'))
        }
        return
      }

      const title = options.model
        ? ` Reviewers (model=${options.model}) `
