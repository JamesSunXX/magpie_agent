...
describe('Config Init', () => {
  ...
  it('should create config file with default content', () => {
    ...
    expect(content).toContain('allow_dangerous_commands: false')
    expect(content).toContain('require_confirmation_for_dangerous: true')
    expect(content).toContain('integration_test: "npm run test:run -- tests/e2e"')
    expect(content).toContain('trd_convergence:')
    expect(content).toContain('max_cycles: 5')
    expect(content).toContain('discuss_rounds: 2')
    expect(content).toContain('auto_back_to_prd: true')
...
`, 'utf-8')
    ...
    expect(result.changes).toContain('Added capabilities.safety defaults.')
    ...
    expect(result.changes).toContain('Added capabilities.loop TRD convergence defaults.')
