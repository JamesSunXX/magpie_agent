...
`src/core/` 放通用运行机制，例如上下文、辩论、状态、仓库访问、`src/core/roles/` 这类共享角色编排基础，以及共享失败分类、失败账本和恢复决策。长流程上下文压缩能力也归在这里（`src/core/debate/context-compaction.ts`），由上层按需接入。它给能力层提供基础积木，不反向依赖具体能力。
...
- 长期记忆：把稳定结论沉淀到用户记忆和项目记忆里；不确定、待确认、低稳定性的条目会被过滤，不进入长期记忆
...
本轮对标新增能力按现有分层落位，不新增横向主干：

- 执行隔离属于 workflow shared runtime 和具体能力的会话产物，配置入口在 `src/platform/config/`。
- 工具按需加载属于能力路由和能力准备阶段，工具清单落到 loop / harness 会话目录。
- 技能管理属于能力准备阶段，先使用本地内置技能和配置开关，不引入远程技能市场。
- 后台任务观测属于 harness-server、CLI/TUI 展示和 workflow 会话摘要，读取结构化事件、工具清单和失败记录，不读取原始大日志。
- 权限、失败暂停和资源保护分别落在命令安全检查、`src/core/failures/` 和 harness-server 队列保护里。
- 飞书链路属于 `src/platform/integrations/im/feishu/` 和 `src/cli/commands/im-server.ts`；它只做发单、确认、状态查询和线程展示，任务真实状态仍以 loop / harness 会话为准。

IM 范围只包含飞书。当前不引入 Slack、微信、企业微信、Telegram 等其他渠道，也不把 IM 变成新的业务编排主线。
...
- 改统一状态：优先看 `src/capabilities/workflows/shared/status-summary.ts`、`src/core/status/` 和 `src/cli/commands/status.ts`
- 改技能目录或技能门禁：优先看 `src/core/skills/` 和 `src/cli/commands/skills.ts`
