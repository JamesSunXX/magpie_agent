...
| IM 回调服务 | `magpie im-server start|status|stop|run` | `src/cli/commands/im-server.ts`、`src/platform/integrations/im/` | 第一阶段先负责飞书人工确认回调；会读取 `integrations.im` 配置，验证飞书事件、按会话去重，并把批准/驳回动作转给现有 loop 人工确认逻辑；线程映射、去重事件和服务状态保存在仓库 `.magpie/im/` |
