...
Magpie 会回复当前会话状态、阶段、失败或等待重试原因、下一步动作，以及本地 inspect 命令。状态内容复用 `magpie status` 的统一摘要规则，不直接拼接原始日志。
...
3. 线程里会收到一张确认卡片。
4. 白名单用户可以直接批准或驳回。
5. 白名单外用户点击按钮时，线程会收到拒绝说明，底层任务不会被修改。
6. 驳回原因和补充说明会一起回写到原确认记录里。
7. `magpie` 继续沿用原会话和原工作区往下跑。
